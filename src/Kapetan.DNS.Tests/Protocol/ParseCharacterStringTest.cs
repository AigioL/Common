...
public sealed class ParseCharacterStringTest
{
    [Fact]
    ...
    public void CharacterStringWithWrongLengthOver()
    {
        ...
        Assert.ThrowsAny<ArgumentException>(() => CharacterString.FromArray(fixture, 0));
    }
    ...
    public void CharacterStringFromLongString()
    {
        ...
        Assert.Equal(new[] { 255, 255, 2 }, result.Select(x =>
        {
            var bytes = new byte[x.Size];
            x.Write(bytes);
            return (int)bytes[0];
        }));
    }
