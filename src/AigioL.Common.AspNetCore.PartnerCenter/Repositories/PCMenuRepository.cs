...
sealed partial class PCMenuRepository<
...
{
    public PCMenuRepository(TDbContext dbContext, IServiceProvider serviceProvider) : base(dbContext, serviceProvider)
    {
    }
    ...
    public async Task<(int rowCount, DbRowExecResult result)> InsertOrUpdateAsync(BMMenuEdit model, Guid userId, Guid tenantId)
    {
        ...
        {
            entity.CreatePCUserId = userId;
            entity.TenantId = tenantId;
        }, onUpdate: (entity) =>
        {
            entity.OperatorPCUserId = userId;
        });
        return r;
    }
...
partial class PCMenuRepository<TDbContext, TUser, TRole, TUserRole> // 菜单权限
{
    public async Task<bool> EditMenuButtonsAsync(Guid menuId, Guid tenantId, params IEnumerable<Guid> buttons)
    {
        ...
        var buttonsE = buttons.Select(x => new PCMenuButton
        {
            MenuId = menuId,
            ButtonId = x,
            TenantId = tenantId,
    ...
    public async Task<List<PCMenuButtonModel>> GetUserMenuAsync(Guid userId, Guid tenantId)
    {
        var q1 = db.Buttons.AsNoTrackingWithIdentityResolution()
            .Where(x => x.TenantId == tenantId)
            .Select(x => new PCButtonModel
            {
                Id = x.Id,
                Name = x.Name,
                Type = x.Type,
    ...
    static PCMenuButtonModel? MapToTreeModel(Dictionary<Guid, IEnumerable<PCButtonModel>>? dict, PCMenu m)
    {
        if (dict == null || !dict.TryGetValue(m.Id, out var btn))
            return null;
        if (btn == null)
            return null;
        PCMenuButtonModel r = new()
        {
            Id = m.Id,
            Key = m.Key,
            Name = m.Name,
    ...
    PCButtonModel? GetMenuSysButtonModel(Dictionary<Guid, PCButtonModel> dict, Guid buttonId)
    {
    ...
    public async Task<List<PCButtonModel>> GetButtonsAsync()
    {
    ...
    public async Task<List<PCButtonModel>> GetRoleMenuButtonsAsync(Guid roleId, Guid menuId, Guid tenantId)
    {
    ...
    internal static string GetControllerName(string controllerName, PCButtonType buttonType) => PermissionAuthorizationRequirement.GetPolicyName(controllerName, buttonType);

    public async Task<bool> AddMenuButtonsAsync(Guid userId, Guid roleId, Guid menuId, Guid tenantId, params IEnumerable<PCButtonModel> buttons)
    {
        ...
        if (!isMenuAny)
        {
            await db.MenuButtonRoles.AddRangeAsync(buttons.Select(x => new PCMenuButtonRole
            {
                ButtonId = x.Id,
                MenuId = menu.Id,
                RoleId = roleId,
        ...
        else
        {
            ...
            if (isParentMenu)
            {
                ...
                List<PCMenuButtonRole> roles = new();
                foreach (var item in menus)
                {
                    ...
                    if (!isAny)
                    {
                        roles.AddRange(buttons.Select(x => new PCMenuButtonRole
                        {
                            ButtonId = x.Id,
                            MenuId = item.Id,
    ...
    public async Task<bool> EditMenuButtonsAsync(string name, Guid userId, Guid roleId, Guid menuId, Guid tenantId, params IEnumerable<PCButtonModel> buttons)
    {
        var menu = await db.Menus.AsNoTrackingWithIdentityResolution()
               .FirstOrDefaultAsync(x => x.Id == menuId && x.TenantId == tenantId, RequestAborted);
        if (menu == null)
...
{
    internal static readonly Expression<Func<PCButton, PCButtonModel>> BtnExpr = it => new()
    {
        Id = it.Id,
        Name = it.Name,
        Type = it.Type,
    ...
    internal static readonly Expression<Func<PCMenu, BMMenuModel>> MenuExpr = item => new()
    {
        Id = item.Id,
        Name = item.Name,
