...
namespace SAM.Picker
{
    internal partial class GamePicker : Form
    {
        ...
        private readonly Lock _LogoLock;
        ...
        private void OnAppDataChanged(ref APITypes.AppDataChanged param)
        {
        ...
        private void DoDownloadList(object sender, DoWorkEventArgs e)
        {
            ...
            var stream = new MemoryStream(Resources.games, false);
            {
        ...
        private void DoDownloadLogo(object sender, DoWorkEventArgs e)
        {
            ...
            try
            {
                using var rsp = httpClient.Send(new HttpRequestMessage(HttpMethod.Get, new Uri(info.ImageUrl!)));
                using var stream = rsp.Content.ReadAsStream();
                {
                    Bitmap bitmap = new(stream);
                    result = new LogoInfo(info.Id, bitmap);
                }
            }
            catch (Exception)
            {
                result = new LogoInfo(info.Id, null!);
            }
            e.Result = result;
        }
