...
public static partial class BMRolesController
{
    ...
    public static void MapBMRoles<TRole>(this IEndpointRouteBuilder b, [StringSyntax("Route")] string pattern = "bm/roles") where TRole : ACRole, new()
    {
    ...
    static async Task<ApiRspAC<List<SelectItemModel<Guid>>?>> GetList(HttpContext context)
    {
        var repo = context.RequestServices.GetRequiredService<IACRoleRepository>();
        var r = await repo.GetSelectAsync();
        return r;
    }

    static async Task<ApiRspAC<PagedModel<ACRoleModel>?>> Get(HttpContext context, int current, int pageSize, string? name = null)
    {
        var repo = context.RequestServices.GetRequiredService<IACRoleRepository>();
    ...
    static async Task<ApiRspAC<bool>> Post<TRole>(HttpContext context, ACRoleModel model, Guid tenantId) where TRole : ACRole, new()
    {
    ...
    static async Task<ApiRspAC<bool>> Put<TRole>(HttpContext context, ACRoleModel model, Guid tenantId) where TRole : ACRole
    {
        var roleManager = context.RequestServices.GetRequiredService<RoleManager<TRole>>();
    ...
    static async Task<ApiRspAC<List<Guid>?>> GetRoleMenus(HttpContext context, Guid roleId, Guid tenantId)
    {
        var repo = context.RequestServices.GetRequiredService<IACRoleRepository>();
