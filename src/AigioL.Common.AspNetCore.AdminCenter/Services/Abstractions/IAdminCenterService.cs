...
public partial interface IAdminCenterService
{
    ...
    /// 获取初始化时的可添加角色名称集合
    /// </summary>
    /// <param name="roleEnumType"></param>
    /// <returns></returns>
#pragma warning disable IL3050 // Calling members annotated with 'RequiresDynamicCodeAttribute' may break functionality when AOT compiling.
    HashSet<string> GetAddRoles(Type roleEnumType) => [.. Enum.GetValues(roleEnumType).Cast<Enum>().Select(x => x.ToString()).Where(x => x != RoleNameAdministrator)];
