...
sealed partial class ACMenuRepository<
...
{
    public ACMenuRepository(TDbContext dbContext, IServiceProvider serviceProvider) : base(dbContext, serviceProvider)
    {
    }

    public async Task<List<ACMenuTreeItem>> GetTreeAsync()
    {
        // 查询顶级菜单（带上它的下一级子菜单列表）
        var query = db.Menus.AsNoTrackingWithIdentityResolution()
            .Where(static x => !x.ParentId.HasValue)
            .Select(static x => new ACMenuTreeItem
            {
                Id = x.Id,
                ParentId = x.ParentId,
                Name = x.Name,
                ...
                Key = x.Key,
                Url = x.Url,
                Children = x.Children!.Select(static x => new ACMenuTreeItem
                {
                    Id = x.Id,
                    ParentId = x.ParentId,
                    Name = x.Name,
    ...
    public async Task<ACMenuModel?> InfoAsync(Guid id)
    {
        var r = await db.Menus.AsNoTrackingWithIdentityResolution()
           .Select(ACMenu.GetExpression())
           .FirstOrDefaultAsync(x => x.Id == id, RequestAborted);
        return r;
    }

    public async Task<(int rowCount, DbRowExecResult result)> InsertOrUpdateAsync(ACMenuEdit model, Guid userId, Guid tenantId)
    {
...
partial class ACMenuRepository<TDbContext, TACUser, TACRole, TACUserRole> // 菜单权限
{
    public async Task<bool> EditMenuButtonsAsync(Guid menuId, Guid tenantId, params IEnumerable<Guid> buttons)
    {
        ...
        var buttonsE = buttons.Select(x => new ACMenuButton
        {
            MenuId = menuId,
            ButtonId = x,
            TenantId = tenantId,
    ...
    public async Task<List<ACMenuButtonModel>> GetUserMenuAsync(Guid userId, Guid tenantId)
    {
        var q1 = db.Buttons.AsNoTrackingWithIdentityResolution()
            .Where(x => x.TenantId == tenantId)
            .Select(x => new ACButtonModel
            {
                Id = x.Id,
                Name = x.Name,
                Type = x.Type,
    ...
    static ACMenuButtonModel? MapToTreeModel(Dictionary<Guid, IEnumerable<ACButtonModel>>? dict, ACMenu m)
    {
        if (dict == null || !dict.TryGetValue(m.Id, out var btn))
            return null;
        if (btn == null)
            return null;
        ACMenuButtonModel r = new()
        {
            Id = m.Id,
            Key = m.Key,
            Name = m.Name,
    ...
    ACButtonModel? GetMenuSysButtonModel(Dictionary<Guid, ACButtonModel> dict, Guid buttonId)
    {
    ...
    public async Task<List<ACMenuModel>> GetRoleTreeAsync()
    {
        var query = db.Menus.AsNoTrackingWithIdentityResolution()
            .Include(x => x.Children)
    ...
    public async Task<List<ACButtonModel>> GetButtonsAsync()
    {
        var query = db.Buttons.AsNoTrackingWithIdentityResolution()
            .Where(x => !x.SoftDeleted)
            .Select(ACButton.GetExpression());
    ...
    public async Task<List<ACButtonModel>> GetRoleMenuButtonsAsync(Guid roleId, Guid menuId, Guid tenantId)
    {
        ...
        var r = await query
            .Select(ACButton.GetExpression()).ToListAsync(RequestAborted);
        return r;
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    internal static string GetControllerName(string controllerName, ACButtonType buttonType) => PermissionAuthorizationRequirement.GetPolicyName(controllerName, buttonType);

    public async Task<bool> AddMenuButtonsAsync(Guid userId, Guid roleId, Guid menuId, Guid tenantId, params IEnumerable<ACButtonModel> buttons)
    {
        ...
        if (!isMenuAny)
        {
            await db.MenuButtonRoles.AddRangeAsync(buttons.Select(x => new ACMenuButtonRole
            {
                ButtonId = x.Id,
                MenuId = menu.Id,
                RoleId = roleId,
        ...
        else
        {
            ...
            if (isParentMenu)
            {
                ...
                List<ACMenuButtonRole> roles = new();
                foreach (var item in menus)
                {
                    ...
                    if (!isAny)
                    {
                        roles.AddRange(buttons.Select(x => new ACMenuButtonRole
                        {
                            ButtonId = x.Id,
                            MenuId = item.Id,
...
file static class _
{
    internal static readonly Comparison<ACMenuTreeItem> menuComparisonBySort = (a, b) => (int)(a.Sort - b.Sort);
}