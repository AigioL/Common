...
public partial interface IACMenuRepository
{
    /// <summary>
    /// 查询树结构
    /// </summary>
    /// <returns></returns>
    Task<List<ACMenuTreeItem>> GetTreeAsync();
    ...
    /// <param name="id"></param>
    /// <returns></returns>
    Task<ACMenuModel?> InfoAsync(Guid id);
    ...
    /// <returns></returns>
    Task<(int rowCount, DbRowExecResult result)> InsertOrUpdateAsync(ACMenuEdit model, Guid userId, Guid tenantId);
}

partial interface IACMenuRepository // 菜单权限
{
    /// <summary>
    /// 获取菜单下的按钮
    /// </summary>
    ...
    /// <param name="tenantId"></param>
    /// <returns></returns>
    Task<List<ACMenuButtonModel>> GetUserMenuAsync(Guid userId, Guid tenantId);

    /// <summary>
    /// 获取角色权限编辑菜单（暂时没有加基础菜单表等无筛选权限）
    /// </summary>
    /// <returns></returns>
    Task<List<ACMenuModel>> GetRoleTreeAsync();

    /// <summary>
    /// 获取系统按钮列表
    /// </summary>
    /// <returns></returns>
    Task<List<ACButtonModel>> GetButtonsAsync();
    ...
    /// <param name="tenantId"></param>
    /// <returns></returns>
    Task<List<ACButtonModel>> GetRoleMenuButtonsAsync(
        Guid roleId,
        Guid menuId,
        Guid tenantId);
