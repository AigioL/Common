...
using AigioL.Common.Repositories.EntityFrameworkCore.Extensions;
...
public abstract class Repository<TDbContext, [DynamicallyAccessedMembers(IEntity.DAMT)] TEntity, [DynamicallyAccessedMembers(IEntity.DAMT)] TPrimaryKey> : Repository<TDbContext, TEntity>, IRepository<TEntity, TPrimaryKey>
    ...
    where TPrimaryKey : IEquatable<TPrimaryKey>
{
    ...
    bool? mIsAutoGeneratedKey;

    public Repository(TDbContext dbContext, IServiceProvider serviceProvider) : base(dbContext, serviceProvider)
    {
    }
    ...
    [Obsolete("use SetDisableByIdAsync in AigioL.Common.Repositories.SourceGenerator")]
    ...
    /// <inheritdoc/>
    public virtual Task<TEditViewModel?> GetEditByIdAsync<TEditViewModel>(TPrimaryKey primaryKey, CancellationToken cancellationToken = default) where TEditViewModel : notnull, IReadOnlyId<TPrimaryKey>
    {
        var query = EntityNoTracking.Where(IRepository<TEntity, TPrimaryKey>.LambdaEqualId(primaryKey));
    ...
    protected virtual bool GetIsAutoGeneratedKey()
    {
    ...
    /// 实体是否使用自动生成的键值
    /// </summary>
    public bool IsAutoGeneratedKey
    {
        get
        {
            mIsAutoGeneratedKey ??= GetIsAutoGeneratedKey();
            return mIsAutoGeneratedKey.Value;
        }
    }
        ...
        CancellationToken cancellationToken = default) where TViewModel : notnull
    {
        ...
        else if (viewModel is IReadOnlyId<TPrimaryKey> km_id)
        {
            primaryKey = km_id.Id;
        }
        ...
        else
        {
            throw new InvalidOperationException(
                $"Type {viewModel.GetType()} must inherit from {typeof(IReadOnlyId<TPrimaryKey>)}.");
        }
        var existingEntity = await FindAsync(primaryKey, cancellationToken);
