...
public partial interface IObjectStorageService
{
    ...
    /// <param name="fileExNoCharDot">文件扩展名（前面不带 . 字符开头！）</param>
    /// <param name="hashHex">哈希值的十六进制表示</param>
    /// <returns>生成的对象存储服务的 Key</returns>
    delegate string GetKeyFuncDelegate(ReadOnlySpan<char> keyPrefix, ReadOnlySpan<char> fileExNoCharDot, ReadOnlySpan<char> hashHex);
    ...
    static bool TryGetContentType(string extension, [MaybeNullWhen(false)] out string? contentType)
    {
        if (extension == null || extension.Length > MediaTypeNames_Mappings_Mappings.MaxKeyLength)
        {
            contentType = null;
            return false;
        }
        ...
        return MediaTypeNames_Mappings_Mappings.values.TryGetValue(extension, out contentType);
    }
}

file static class MediaTypeNames_Mappings_Mappings
{
    public static readonly IDictionary<string, string> values;

    public static readonly int MaxKeyLength;

    static MediaTypeNames_Mappings_Mappings()
    {
#if !NO_ASPNETCORE
        values = new global::Microsoft.AspNetCore.StaticFiles.FileExtensionContentTypeProvider().Mappings;
#else
        values = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase) // https://github.com/dotnet/aspnetcore/blob/v11.0.0-rc.1.26425.128/src/Middleware/StaticFiles/src/FileExtensionContentTypeProvider.cs#L22
        {
            { ".323", "text/h323" },
            { ".3g2", "video/3gpp2" },
            { ".3gp2", "video/3gpp2" },
...
#endif
        MaxKeyLength = values.Keys.Max(static x => x.Length);
    }
}