...
public sealed class SteamUserStats011 : NativeWrapper<ISteamUserStats007>
{
    #region RequestCurrentStats

    public unsafe bool RequestCurrentStats()
    {
        var __retVal = ((delegate* unmanaged[Thiscall]<nint, sbyte>)Functions.RequestCurrentStats)(ObjectAddress);
        return __retVal != 0;
    }
    ...
    public unsafe nint RequestGlobalAchievementPercentages()
    {
        var __retVal = ((delegate* unmanaged[Thiscall]<nint, nint>)Functions.RequestGlobalAchievementPercentages)(ObjectAddress);
        return __retVal;
    }
    ...
    public unsafe bool GetAchievementAchievedPercent(string name, out float percent)
    {
        int expectedByteCount = Encoding.UTF8.GetMaxByteCount(name.Length) + 1;
        ...
        try
        {
            ...
            bytes[len..].Clear();
            sbyte __retVal;
            fixed (float* __percent_native = &percent)
            {
                if (isArrayPool)
                {
                    var handle = GCHandle.Alloc(array, GCHandleType.Pinned);
                    try
                    {
                        __retVal = ((delegate* unmanaged[Thiscall]<nint, nint, float*, sbyte>)Functions.GetAchievementAchievedPercent)(ObjectAddress, handle.AddrOfPinnedObject(), __percent_native);
                    }
                    finally
                    {
                        handle.Free();
                ...
                else
                {
                    fixed (byte* p = bytes)
                    {
                        __retVal = ((delegate* unmanaged[Thiscall]<nint, nint, float*, sbyte>)Functions.GetAchievementAchievedPercent)(ObjectAddress, (nint)p, __percent_native);
                    }
                }
            }
            return __retVal != 0;
