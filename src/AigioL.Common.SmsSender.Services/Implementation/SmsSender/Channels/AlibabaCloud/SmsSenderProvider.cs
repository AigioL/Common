...
using System.Text;
using SmsOptions = AigioL.Common.SmsSender.Models.Channels.AlibabaCloud.SmsAlibabaCloudOptions;

namespace AigioL.Common.SmsSender.Services.Implementation.SmsSender.Channels.AlibabaCloud;
...
public partial class SmsSenderProvider : SmsSenderBase, ISmsSender
{
    ...
    public override async Task<ISendSmsResult> SendSmsAsync(string number, string message, ushort type, CancellationToken cancellationToken)
    {
        ...
        ArgumentNullException.ThrowIfNull(signName);
...
#region 业务参数
           ...
           { "SignName", signName }, // 短信签名 正式环境要写入APP的名称。
           { "TemplateCode", template_code }, // 短信模板ID，发送国际/港澳台消息时，请使用国际/港澳台短信模版
    ...
    [LoggerMessage(
        ...
        Message =
"""
调用阿里云短信接口失败，手机号码：{phoneNumber}，短信内容：{message}，短信类型：{type}，HTTP 响应状态码：{httpStatusCode}
""")]
