using AigioL.Common.Primitives.Columns;
using AigioL.Common.Primitives.Entities.Abstractions;
using Microsoft.EntityFrameworkCore;
using System.ComponentModel.DataAnnotations;
using System.ComponentModel.DataAnnotations.Schema;

namespace AigioL.Common.AspNetCore.AppCenter.Analytics.Entities.AnalysisLogs.Summaries;

/// <summary>
/// 分析事件日志汇总表实体类
/// </summary>
[Table("AnalysisEventLogSummaries")]
public partial class AnalysisEventLogSummary :
    Entity<Guid>,
    ICreateTime,
    INEWSEQUENTIALID
{
    /// <summary>
    /// 事件名
    /// </summary>
    [Required]
    [Comment("事件名")]
    public required string EventName { get; set; }

    /// <summary>
    /// 应用 Id
    /// </summary>
    [Comment("应用 Id")]
    public Guid AppId { get; set; }

    /// <summary>
    /// 统计值
    /// </summary>
    [Comment("统计值")]
    public int StatisticalValues { get; set; }

    /// <summary>
    /// 统计时间
    /// </summary>
    [Comment("统计时间")]
    public DateTimeOffset StatisticalTime { get; set; }

    /// <inheritdoc/>
    [Comment("创建时间")]
    public DateTimeOffset CreateTime { get; set; }

    /// <summary>
    /// AppVersion
    /// </summary>
    [Required]
    [Comment("AppVersion")]
    public required string AppVersion { get; set; }

    /// <summary>
    /// 是否月统计
    /// </summary>
    [Comment("是否月统计")]
    public bool IsTheMonthlyStatistics { get; set; }

    public virtual List<EventRelatedPropertieSummary> EventRelatedPropertie { get; set; } = null!;
}
